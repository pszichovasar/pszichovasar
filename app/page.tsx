...
export default function Home() {
  ...
  // Для розового фона и видео/текста — простые стейты для opacity
  ...
  // Единицы скролла:
  // 0..1  — розовая секция
  // 1..2  — ряд едет справа налево (появляется из правого края, уезжает в левый)
  // 2..3  — секция designer
  ...
  const applyAnimations = (scrollY: number) => {
    const unit = scrollY / SCROLL_PER_UNIT; // 0..3

    // --- РОЗОВЫЙ ФОН: исчезает в диапазоне unit 0.8..1.2 ---
    const pink = Math.max(0, 1 - Math.max(0, (unit - 0.8) / 0.4));
    setPinkOpacity(pink);

    // --- РЯД КАРТИНОК ---
    // unit 1..2: ряд проезжает справа налево
    // при unit=1: левый край ряда на правом краю экрана (x = vw)
    // при unit=2: правый край ряда на левом краю экрана (x = -rowWidth)
    const track = trackRef.current;
    if (track) {
      const vw = window.innerWidth;
      const vh = window.innerHeight;
      const imgSize = Math.floor((vh - GAP * 2) / 1); // один ряд — размер под высоту
      const tileSize = Math.min(imgSize, Math.floor((vh - GAP * 4) / 5)); // примерно 1/5 экрана
      const rowWidth = (tileSize + GAP) * IMAGES.length + GAP;

      // Начало: x = vw (ряд полностью за правым краем)
      // Конец:  x = -rowWidth (ряд полностью за левым краем)
      const startX = vw;
      const endX = -rowWidth;

      if (unit < 1) {
        // До анимации — ряд за правым краем, невидим
        ...
        // После — ряд за левым краем
        track.style.opacity = "0";
        track.style.transform = `translate3d(${endX}px, 0, 0)`;
      }
    }

    // --- ВИДЕО И ТЕКСТ: появляется при unit > 2.2 ---
    const tPhase = Math.max(0, Math.min((unit - 2.2) / 0.5, 1));
    setVideoOpacity(tPhase);
    setTextOpacity(tPhase);
    setTextY((1 - tPhase) * 40);

    if (videoRef.current) {
      videoRef.current.style.opacity = tPhase.toString();
    }
    if (textRef.current) {
  ...
  // Инициализация — ряд за правым краем
  useEffect(() => {
    if (trackRef.current) {
      trackRef.current.style.opacity = "0";
      trackRef.current.style.transform = `translate3d(${window.innerWidth}px, 0, 0)`;
    }
    if (textRef.current) {
  ...
  // Размер тайла — 1/5 высоты экрана
  const tileSize = typeof window !== "undefined"
    ? Math.floor((window.innerHeight - GAP * 4) / 5)
    : 140;
  ...
  return (
    <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "clamp(12px,2vw,15px)", flexGrow: 1, justifyContent: "center" }}>
              {[{ label: "YOUR NAME", key: "name" as const, type: "text" }, { label: "EMAIL", key: "email" as const, type: "email" }].map(({ label, key, type }) => (
                <div key={key}>
                  ...
                  <input type={type} className="card-input" disabled={isSending} value={form[key]}
                    onChange={(e) => setForm({ ...form, [key]: e.target.value.toUpperCase() })} style={inputStyle} />
                </div>
              ))}
              <div>
                <label className="card-label" style={labelStyle}>SERVICE</label>
                <select className="card-input" style={{ ...inputStyle, cursor: "pointer", appearance: "none" }}
              ...
              <div>
                ...
                <textarea ref={textareaRef} className="card-input" disabled={isSending} value={form.message}
                  onChange={handleMessageChange} rows={1}
      ...
      <main style={{ position: "fixed", width: "100vw", height: "100vh", top: 0, left: 0, overflow: "hidden" }}>
        ...
        {/* СЕКЦИЯ 2: ОДИН РЯД КАРТИНОК */}
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", zIndex: 3, overflow: "hidden", pointerEvents: "none" }}>
          <div
            ref={trackRef}
            style={{
              display: "flex",
